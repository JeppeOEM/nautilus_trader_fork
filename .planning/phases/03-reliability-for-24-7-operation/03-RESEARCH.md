...
**(2) REL-02 has a precise, framework-supported shutdown seam.** On SIGTERM the live runner calls `TradingNode.stop()` → `kernel.stop_async()`, which calls `self._trader.stop()` (firing every strategy's `on_stop()` hook) **before** `self._close_writer()` closes the kernel `StreamingFeatherWriter`. Inside `Strategy._stop()`, `on_stop()` runs **before** the strategy's own timers are cancelled. So an `on_stop()` override is the correct place to (a) flush the strategy-owned funding writer and (b) run a final `_convert_stream`-style `convert_stream_to_data()` pass while the kernel writer's feather data is still on disk. The just-landed non-disjoint-intervals fix (commit `c98b1c0f80`) makes both the periodic mid-day case and the same-day-restart case safe by trimming each conversion to rows newer than the last-converted interval. `[VERIFIED: nautilus_trader/system/kernel.py, nautilus_trader/common/actor.pyx]`
...
|---------|---------|---------|--------------|
| `nautilus_trader` (in-repo build) | pinned (this repo) | `Strategy.on_stop`, `Clock.set_timer`, `StreamingFeatherWriter`, `ParquetDataCatalog.convert_stream_to_data`, Bybit adapter | This is the project framework; all Phase 3 mechanisms ship inside it. `[VERIFIED: codebase]` |
...
| `on_stop()` final convert | Rely solely on next-process first-conversion-cycle to recover the tail | The trimming fix does make next-process recovery safe even on SIGKILL, but REL-02 explicitly requires a flush/convert *on shutdown*; `on_stop` satisfies the requirement literally and shrinks the recovery window. Do both (belt-and-suspenders). |
...
**Version verification:** Not applicable — no external packages. All symbols (`Strategy.on_stop`, `Clock.set_timer`, `convert_stream_to_data`, Bybit WS reconnect) are verified present in the in-repo build by direct source read (see Sources).
...
```
            ...
            |          - convert_stream_to_data() final pass         |
...
def on_stop(self) -> None:
    ...
    # Reuse the same per-type conversion the periodic timer uses. A transient
    # per-type error must be swallowed so it never blocks process shutdown.
    self._run_conversion()  # extract the try/except loop body of _convert_stream
```
**Note:** `on_stop()` must NOT call `convert_stream_to_data` in a way that raises out of the hook — wrap per-type in try/except exactly as `_convert_stream` already does, so a single type's transient error cannot stall the shutdown sequence. `[VERIFIED: kernel.py, actor.pyx]`
...
- **Closing/flushing the kernel `"*"` writer from `on_stop`:** The strategy does not own and cannot reach `kernel._writer`. The kernel closes it itself in `_close_writer()` AFTER `on_stop`. Only flush the *strategy-owned* funding writer; rely on the final `convert_stream_to_data()` to capture the kernel writer's already-flushed feather tail.
...
| Same-day restart de-dup of converted intervals | Manual "what did I already convert?" bookkeeping | `convert_stream_to_data` trimming (fixed in `c98b1c0f80`) | The catalog now trims each conversion to rows newer than the last-converted interval; no app-level bookkeeping needed. |
...
|----------|-------------|------------------|
| Stored data | `catalog/streaming/live/{instance_id}/{type}/*.feather` (kernel "*" writer) and `.../funding/...feather` (strategy funding writer). On same-day restart with the FIXED `instance_id` (`8f1b9c2e-...`), the previous run's un-rotated feather files are re-read by the new process. | Verify (test) that the new process's first `convert_stream_to_data` trims correctly to the un-converted tail — no gap, no overlap. The trimming fix (`c98b1c0f80`) handles this; Phase 3 adds the `on_stop` pass to shrink the window. |
...
**The canonical question (same-day restart):** After SIGTERM→`on_stop` final convert, the next process restart re-reads the same un-rotated feather files; the trimming fix ensures only rows newer than the last-converted parquet interval are persisted. **Verified by source read of `_convert_feather_table_to_parquet` (parquet.py:2597-2630) and the resolved debug note.** A SIGKILL (no `on_stop`) is also safe for *converted* data because the next process recovers the tail on its first conversion cycle — but any feather rows not yet flushed to disk at SIGKILL (the <1s un-flushed buffer) are the only true loss window, which `on_stop` on a clean SIGTERM eliminates.
...
**How to avoid:** In `on_stop`, only flush the *strategy-owned* funding writer, then call `convert_stream_to_data()`. The kernel writer self-flushes on a ≤1s cadence (every `write()` calls `check_flush()` with `flush_interval_ms=1000`), so its feather tail is on disk and convertible. `[VERIFIED: writer.py:276, 568-576; kernel.py:1101, 1454-1456]`
**Warning signs:** `AttributeError` on `kernel`, or a final parquet that's missing the last ~second of data despite `on_stop` running.

### Pitfall 2: Non-disjoint-intervals ValueError on the restart conversion (now fixed — verify, don't re-fix)
**What goes wrong:** A naive same-day-restart conversion of an un-rotated feather file raises `ValueError: ... would create non-disjoint intervals` (the original bug in `.planning/debug/resolved/non-disjoint-intervals.md`).
**Why it happens:** The feather file is only rotated daily (SCHEDULED_DATES), so a restart re-reads rows already converted by the previous run, and the recomputed `(start, end)` overlaps the existing parquet interval.
**How to avoid:** Nothing to build — `_convert_feather_table_to_parquet` now trims the re-read table to rows with `ts_init > max(existing interval end)` before computing `(start, end)` (parquet.py:2597-2614). Phase 3 should *prove this holds across a real restart* with a test (write→convert→append→convert mirrors `test_repeated_conversion_with_new_appended_rows_does_not_raise`), not re-implement it.
**Warning signs:** Re-introducing manual interval bookkeeping in the strategy; tests that expect a raise on the second conversion. `[VERIFIED: parquet.py, resolved debug note]`
...
```python
# Source: scripts/bybit_recorder/strategy.py _convert_stream (276-309) — extract the
# conversion loop so both the timer callback and on_stop can call it.
...
def on_stop(self) -> None:
    self._run_conversion()  # final flush+convert before the kernel closes its writer
```
...
|--------------|------------------|--------------|--------|
| Same-day restart conversion raised non-disjoint ValueError | `convert_stream_to_data` trims to the un-converted tail | commit `c98b1c0f80` (2026-06-14) | Same-day restart and periodic mid-day conversion are both safe; REL-02 no longer needs manual interval handling. |
...
|------------|------------|-----------|---------|----------|
| `nautilus_trader` in-repo build (on_stop, set_timer, convert_stream_to_data, Bybit WS) | REL-02/03/04 | ✓ | this repo (pinned) | — |
...
|--------|----------|-----------|-------------------|-------------|
| REL-02 | `on_stop()` runs a final flush + per-type `convert_stream_to_data` (un-converted feather tail lands in parquet) | unit | `pytest tests/unit_tests/persistence/recorder/test_recorder_strategy.py -k on_stop -x` | ❌ Wave 0 |
| REL-02 | Same-day restart with same `instance_id`: append-then-reconvert produces no gap/overlap (trimming holds) | unit | `pytest tests/unit_tests/persistence/recorder/test_recorder_conversion.py -k "repeated_conversion or double_conversion" -x` | ✅ (existing regression test covers the core contract; add an explicit restart-shaped case) |
...
### Wave 0 Gaps
- [ ] `tests/unit_tests/persistence/recorder/test_recorder_strategy.py` — add `on_stop` final-convert test (spy/assert `convert_stream_to_data` called per type; assert funding writer flushed) and heartbeat/stale-threshold tests using `TestClock` to advance time past/under thresholds.
- [ ] Test for `on_funding_rate` updating last-seen BEFORE the dedup early-return (Pitfall 4).
- [ ] (optional) An explicit "restart-shaped" conversion test that re-opens a new `convert_stream_to_data` pass over a feather file with leftover unconverted rows (mirrors `test_repeated_conversion_with_new_appended_rows_does_not_raise`).
...
| Silent data loss on ungraceful kill (SIGKILL) | Repudiation / data integrity | `on_stop` shrinks the loss window on SIGTERM; trimming fix makes next-process recovery of converted data safe. Document the residual <1s un-flushed window. |
...
- `nautilus_trader/persistence/catalog/parquet.py` — `convert_stream_to_data` (2523-2573) and the trimming fix in `_convert_feather_table_to_parquet` (2597-2630).
