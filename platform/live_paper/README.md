...
- The `nautilus-trader-base` image built once — from `platform/`:
  ```bash
...
- Redis running — `live_paper` needs it for `bots:status`/`bots:control` and the
  ...
  ```bash
  docker compose -f platform/docker-compose.yml up -d redis
  ```
...
```bash
mkdir -p platform/live_paper/data
```
...
Edit `platform/live_paper/config.toml`:
...
**Logs:**
```bash
docker compose -f platform/docker-compose.yml logs -f live-paper
...
Fill history in `platform/live_paper/data/fills.db` and Cache state in Redis both persist
...
`/etc/passwd` entry.) `make test-live-paper` from `platform/` runs the same command.
...
| Symptom | Cause | Fix |
|---|---|---|
| `bots:history refresh cycle failed, keeping stale data: unable to open database file` | `live_paper/data/` is root-owned | `sudo rm -rf platform/live_paper/data && mkdir platform/live_paper/data` (parent dir is user-owned, so no `sudo` needed for the `mkdir` itself if you can remove the root-owned one) |
