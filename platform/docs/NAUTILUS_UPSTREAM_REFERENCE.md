# NautilusTrader Upstream Reference

Lookup-only reference for the upstream `nautilus_trader` / `crates/` codebase this fork
wraps. It is **not** loaded automatically. Read it when you need upstream conventions,
crate layout or engine architecture — for example when reading adapter source under
`crates/adapters/` to understand wire behaviour, or when tracing a bug into
`nautilus_trader/live/`.

The fork rule still applies: `nautilus_trader/` and `crates/` are never modified. Project
work lives under `platform/` and is governed by `CLAUDE.md` and `platform/CLAUDE.md`.

Version pins listed below were captured on 2026-09-21; `Cargo.lock`, `uv.lock` and
`rust-toolchain.toml` are the source of truth.
