...
# -------------------------------------------------------------------------------------------------
"""
Bybit market-data collector: own asyncio loop, ingest queue, buffer and flush timer over
`BybitClient`, writing to the shared `ParquetDataCatalog` (Nautilus's own write_data(), so
it loads straight into backtests). No TradingNode/Strategy/DataEngine.

Mirrors `dydx_collector.collector`'s venue-agnostic shape only (Story 19.3, DESIGN-01: a
sibling, not a shared base). Deliberately left out vs dYdX:
...
existing routes serve Bybit instruments with zero per-route code (Story 19.2 AC3).
"""
...
_STALE_BOOK_NS = 5_000_000_000
_CROSSED_RESYNC_NS = 10_000_000_000
...
class Collector:
    def __init__(self, config: CollectorConfig) -> None:
        ...
        self._client = BybitClient(
            ...
            environment=BybitEnvironment.TESTNET if config.environment == "testnet" else BybitEnvironment.MAINNET,
        )
    ...
    def _process_data(self, data: Any) -> None:
        if isinstance(data, OrderBookDeltas):
            ...
            pass  # the ticker/orderbook quotes are derivable from the snapshots; not persisted
    ...
    def _sample(self, now_ns: int) -> tuple[list[DydxSecondSnapshot], list[str]]:
        """One snapshot per healthy book, plus the ids whose book has been crossed too long."""
        ...
        for iid in self._config.instruments:
            ...
            if bid.as_double() >= ask.as_double():
                ...
                if now_ns - since > _CROSSED_RESYNC_NS:
                    to_resync.append(iid)
                continue
            self._crossed_since_ns.pop(iid, None)
            if now_ns - self._last_book_update_ns.get(iid, 0) > _STALE_BOOK_NS:
                logger.warning("Stale book for %s, skipping sample", iid)
                continue
            batch.append(self._snapshot(iid, book, trades, now_ns))
        return batch, to_resync
    ...
    async def _second_loop(self) -> None:
        while not self._stop.is_set():
            ...
            batch, to_resync = self._sample(time.time_ns())
            for snapshot in batch:
                self._buffer[(DydxSecondSnapshot, str(snapshot.instrument_id))].append(snapshot)
            for iid in to_resync:
                logger.error("Book for %s crossed > %ds, resubscribing", iid, _CROSSED_RESYNC_NS // 10**9)
                self._live_books.pop(iid, None)
                self._crossed_since_ns.pop(iid, None)
                try:
                    await self._client.resync_orderbook(iid)
                except Exception:
                    logger.exception(f"Failed to resync {iid}")
    ...
    async def _open_interest_loop(self) -> None:
        while not self._stop.is_set():
            ...
            try:
                wanted = set(self._config.instruments)
    ...
    async def run(self) -> None:
        ...
        if unknown:
            logger.warning("Configured instruments not found on Bybit, skipping: %s", sorted(unknown))
        ...
        tasks = [
            ...
            for loop in (self._ingest_loop, self._flush_loop, self._second_loop, self._open_interest_loop)
        ]
