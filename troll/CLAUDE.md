...
- **DATA-05** — **No data loss, where it can be prevented. Never accept a silent gap as "small" or "just a cache."** This covers derived data too (e.g. `DydxMinuteRollup`): a missing minute understates the wide candle built from it, so it counts. Any seam that can drop data (restart, shutdown, chunk/range boundary, lookahead-dependent close) must be closed by design or have a documented, tested way to fill it (e.g. an idempotent backfill that skips existing rows). A loss that genuinely can't be prevented must be named explicitly, with how to detect and repair it -- never left implicit. **Why:** the builder only closes a minute on the next minute's first snapshot, so the minute in progress at a collector restart is never emitted, and the first minute of a run that starts mid-minute is deliberately *not* emitted either (a knowingly understated row could never be corrected by the append-only, skip-existing backfill). Both leave an honest hole that `backfill_minute_rollup` fills from raw 1s -- idempotently, one disjoint run per gap. Two edges stay open by design: without `--end`, the data's last minute has no successor snapshot and is skipped until a later run; and until the backfill is run, wide candles are missing those minutes (Story 16.2 does not fall back per-minute inside a covered range).
