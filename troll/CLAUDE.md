...
These rules govern all code under `troll/`: `collector_core/`, the venue collectors
...
them. **Exception:** `troll/live_paper/` is a separate, structurally isolated module
...
- **DATA-01** — **Correct data is the #1 priority. Never display stale or fabricated values as live market data.** If data is genuinely unavailable (e.g., during WS reconnect recovery when the Rust client re-subscribes instruments at 2/sec), the gap must be flagged visually rather than papered over with a flatline. **Implementation:** `collector_core/collector.py`'s `_second_loop` uses `config.stale_book_seconds` (5 s) to skip snapshot emission when no `OrderBookDeltas` have arrived for an instrument; `data_api/routes/snapshots.py` breaks the series at timestamp gaps > `_SNAPSHOT_GAP_THRESHOLD_MS` (2500 ms) so the chart renders an honest gap instead of a misleading horizontal line. When adding new data sources or display paths, apply the same principle: skip or flag, never silently perpetuate stale state.
