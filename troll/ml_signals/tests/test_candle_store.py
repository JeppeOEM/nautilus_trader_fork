...
def _second(sec: int, price: float | None, volume: float = 1.0) -> SimpleNamespace:
    ...
    return SimpleNamespace(
        ...
        open_price=price, high_price=price + 0.5 if traded else None,
        low_price=price - 0.5 if traded else None, close_price=price + 0.1 if traded else None,
        buy_volume=volume if traded else 0.0, sell_volume=0.25 if traded else 0.0,
    )
...
def _fixture() -> list[SimpleNamespace]:
    ...
    for sec in range(6 * 3600):
        ...
        rows.append(_second(sec, rng.uniform(90, 110) if rng.random() < 0.05 else None, rng.uniform(0.1, 5)))
    return rows
...
def _check_matches_raw_aggregation(db, rows: list[SimpleNamespace]) -> None:
    for bar in candle_store.BAR_SECONDS:
        ...
        for g, w in zip(got, want, strict=True):
            ...
            seconds = sum(1 for r in rows if r.ts_event // 1_000_000 // (bar * 1000) * (bar * 1000) == g["t"])
...
def test_rebuild_repairs_a_hole_and_is_idempotent(tmp_path: Path) -> None:
    db = candle_store.connect_rw(str(tmp_path / "c.db"))
    rows = _fixture()
    candle_store.apply_seconds(db, _IID, [r for r in rows if not 3000 <= (r.ts_event - _DAY0_MS * 1_000_000) // _SEC_NS < 3600])
    hour0 = next(c for c in _stored(db, 3600) if c["t"] == _DAY0_MS)
    assert hour0["seconds_observed"] == 3000  # the missed 10 minutes are visibly absent before the rebuild
...
def test_rebuild_refuses_the_open_day_unless_told(tmp_path: Path) -> None:
    ...
    now_row = SimpleNamespace(
        ts_event=today_ms * 1_000_000, open_price=1.0, high_price=1.0, low_price=1.0, close_price=1.0,
        buy_volume=1.0, sell_volume=0.0,
    )
    assert candle_store.rebuild(db, _IID, [now_row], today_ms, today_ms + 86_400_000) == 0
    assert candle_store.rebuild(db, _IID, [now_row], today_ms, today_ms + 86_400_000, allow_open_day=True) == 1
