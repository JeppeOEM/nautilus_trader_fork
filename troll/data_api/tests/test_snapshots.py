...
def test_price_series_rows_matches_dashboard_oracle_byte_identical() -> None:
    """
    SSOT-03 (`dashboard.py`'s own docstring: this math "must never be reimplemented a
    second time"): proves the ported `snapshots._price_series_rows` produces byte-identical
    output to the original `ml_signals.dashboard._price_series_rows` on identical input.

    `ml_signals.dashboard` is imported here ONLY as a test-only oracle -- never at runtime
    by `data_api` (see `routes/snapshots.py`'s module docstring). `dashboard.py` still
    exists until Story 15.10; if it is deleted before then, this check must be replaced with
    a hand-verified expected-value assertion, never simply removed.
    """
    from ml_signals.dashboard import _price_series_rows as oracle_price_series_rows
    ...
    snaps = [
        ...
        # A >2.5s gap follows -- exercises the gap-marker branch in both copies.
        {
            "bid_prices": [102.0],
            "bid_sizes": [1.0],
            "ask_prices": [103.0],
        ...
        # A crossed row -- exercises the skip branch in both copies.
        {
            "bid_prices": [110.0],
            "bid_sizes": [1.0],
            "ask_prices": [105.0],
        ...
        },
        {
            ...
            "buy_volume": 2.0,
            "sell_volume": 4.0,
            "ts_event": 5_200_000_000,
        },
    ]

    assert snapshots_routes._price_series_rows(snaps) == oracle_price_series_rows(snaps)
