...
- [ ] Task 1 — move + generalise (AC: #1)
  - [ ] 22.3 already moved the module and its test to `collector_core/`; update `troll/Makefile` `consolidate` target and the audit runbook line. The module already has no dYdX-specific code: `leaf_dirs` walks every `data/<type>/<instrument>` directory, so Bybit/Hyperliquid leaves are covered by construction — prove it with a test that seeds a `.BYBIT` and a `.HYPERLIQUID` instrument (`DydxSecondSnapshot` rows, same class for all venues) plus one `MarkPriceUpdate` leaf and asserts one file per closed day in each.
