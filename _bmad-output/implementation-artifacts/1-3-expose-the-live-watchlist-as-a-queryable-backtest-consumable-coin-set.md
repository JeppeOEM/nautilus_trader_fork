...
- [x] Task 1 — Fix the real gap found during research: stale/unsubscribed instruments never leave `_LIVE_FAST` (AC: #2)
  ...
  - [x] **Also a scope decision:** do NOT change `_rankings_json()`'s existing behavior (the human-facing rankings table). Per `troll/CLAUDE.md`'s DATA-01 ("flag, don't hide, stale data"), it's arguably *better* for a human dashboard viewer to keep seeing a now-stale row (their eyes can judge it looks frozen) than to have it vanish silently. This story's freshness filter applies only to the new Watchlist-specific output (Task 2), which has a different consumer (an automated backtest script that must never silently include a dead coin).

- [x] Task 2 — Add a `_watchlist_ids()` function and `/api/watchlist` route (AC: #1, #2, #4)
  - [x] `_watchlist_ids() -> list[str]` in `ml_signals/dashboard.py`: `now_ns = time.time_ns()`; filter `_merged_rows()` (existing helper, `dashboard.py:144-146`) to only fresh instruments via `_is_fresh`; sort by `_VOLUME_24H.get(iid, 0.0)` descending — same sort key `_rankings_json()` already uses (`dashboard.py:812`, added by Story 1.2), reused for consistency, not reinvented; return just the `instrument_id` strings
  - [x] New `async def watchlist_json_handler(request: web.Request) -> web.Response` returning `web.Response(text=json.dumps({"instrument_ids": _watchlist_ids()}), content_type="application/json")` — mirrors `rankings_json_handler`'s existing shape (`dashboard.py:972`)
  - [x] Register `app.router.add_get("/api/watchlist", watchlist_json_handler)` in `make_app()` (`dashboard.py:1206-1238`, alongside the other `add_get` calls)
  - [x] Confirm `_watchlist_ids()` touches no `ParquetDataCatalog`/catalog_path access at all — it only reads `_LIVE_FAST`/`_VOLUME_24H`, already-in-memory dashboard state (trivially satisfies AC #4/NFR3)

- [x] Task 3 — Add a lean, importable fetch helper for backtest/notebook consumers (AC: #1, #3)
  - [x] New file `troll/ml_signals/watchlist.py` (deliberately NOT added to `dashboard.py` — a backtest script or Jupyter notebook importing this must not transitively pull in `dashboard.py`'s aiohttp/plotly/redis/indicator import weight just to fetch a coin list)
  - [x] `fetch_watchlist(dashboard_url: str = "http://127.0.0.1:8765") -> list[str]`: plain stdlib `urllib.request` GET against `f"{dashboard_url}/api/watchlist"`, `timeout=10`, `json.load(response)["instrument_ids"]` — mirror `dashboard.py:1148-1163`'s `_fetch_volume_24h_json` request-construction pattern (a `Request` object + `urlopen` + `# noqa: S310` for the fixed-scheme URL), but note this hits the *local* dashboard, not an external host, so no special `User-Agent` header is required the way dYdX's indexer needs one
  - [x] Requires the dashboard process to be running and reachable at `dashboard_url` — this is an explicit precondition (the live Watchlist only exists in the running dashboard's memory), not a bug; document it in the function's docstring so a caller isn't surprised by a connection error when the dashboard isn't up

- [x] Task 4 — Prove the returned coin-set is BacktestDataConfig-compatible (AC: #3)
  - [x] Test: given a stubbed `fetch_watchlist()`-shaped `list[str]` (e.g. `["BTC-USD-PERP.DYDX", "ETH-USD-PERP.DYDX"]`), build `[BacktestDataConfig(catalog_path="troll/dydx_collector/catalog", data_cls=TradeTick, instrument_id=InstrumentId.from_str(iid)) for iid in ids]` — confirmed via a real `.venv` check that `BacktestDataConfig` construction does **not** eagerly touch disk/the catalog path (no I/O at construction time), so this test needs no real catalog fixture, just real Nautilus objects (satisfies "never mock Nautilus internals" — only `fetch_watchlist`'s own HTTP transport is stubbed, not any Nautilus type)
  - [x] Assert one `BacktestDataConfig` is produced per watchlist entry, with `.instrument_id == InstrumentId.from_str(iid)` for each — this is the literal proof that "accepts the returned coin-set as-is... no manual per-coin editing" holds
  - [x] **Do not** wire this into `backtest_dydx.py`'s actual `run()` function or attempt a real multi-coin `BacktestNode` execution — that is explicitly Epic 2 Story 2.4's scope ("multi-coin backtest runs across the live Watchlist"), not this story's

- [x] Task 5 — Tests for Tasks 1-3 (AC: #1, #2, #4)
  - [x] `_is_fresh`: fresh entry (recent `ts`) → `True`; stale entry (`ts` older than `_WATCHLIST_STALE_NS`) → `False`; missing `iid` → `False`
  - [x] `_watchlist_ids()`: returns only fresh instruments, sorted by descending `_VOLUME_24H`; a stale entry present in `_LIVE_FAST` (old `ts`) is excluded from the result but a fresh one with no `_VOLUME_24H` entry still sorts (as 0.0, consistent with Story 1.2's existing missing-volume convention) — proves AC #2's "no longer present" without needing to touch `_LIVE_FAST` itself
...
- `PYTHONPATH=troll .venv/bin/python -m pytest troll/ml_signals/tests/test_watchlist.py troll/ml_signals/tests/test_dashboard_rankings.py -q` → 18 passed (10 pre-existing + 8 new)
- `PYTHONPATH=troll .venv/bin/python -m pytest troll/dydx_collector troll/ml_signals -q` → 186 passed (up from 178), 1 failed (the same pre-existing, out-of-scope `test_ofi_strategy.py::test_ofi_strategy_generates_long_entry_on_bid_pressure`, reconfirmed unrelated)
- `ruff check`/`ruff format --check` on `troll/ml_signals/watchlist.py` and `troll/ml_signals/tests/test_watchlist.py` (both new files) → 0 findings after fixing an unused-`noqa`, an `__enter__` return-type (`PYI034`), and an import-sort/format pass
- `ruff check`/`ruff format --check` on `troll/ml_signals/dashboard.py` and `troll/ml_signals/tests/test_dashboard_rankings.py` → confirmed via `git stash` diff: baseline 17 pre-existing findings → 18 after this story (net +1). The +1 is `_watchlist_ids()`'s docstring triggering `D213` ("multi-line summary should start at second line") — deliberately left as-is because it matches the exact docstring style already used by 4 other functions in this same file (including `_rankings_json()`, written in Story 1.2), so "fixing" only the new function would make it stylistically inconsistent with its immediate neighbor. Two other candidate new findings (`D401` imperative-mood on `_is_fresh`/`_watchlist_ids`) were fixed by rewording, not left in.
- `.venv/bin/python -c "from nautilus_trader.backtest.node import BacktestDataConfig; ..."` → confirmed `BacktestDataConfig` construction performs no I/O (no real catalog fixture needed for Task 4's test)

### Completion Notes List

- **Fixed the real gap found during story creation:** `_LIVE_FAST[iid]` was set on every ingest but never deleted anywhere in `dashboard.py`. Added `_is_fresh()` (a read-time freshness check, `_WATCHLIST_STALE_NS = 30s`, citing `troll/CLAUDE.md`'s OBS-01 threshold) rather than deleting dict entries — deletion was considered and rejected (Dev Notes) since the dict is bounded by dYdX's small, fixed market count and deleting risked losing indicator continuity across a brief reclassify-then-return.
- **`_watchlist_ids()`** filters `_merged_rows()` through `_is_fresh` and sorts by the same `_VOLUME_24H` key `_rankings_json()` already established in Story 1.2 — no new sort logic invented. Deliberately does **not** touch `_rankings_json()`'s own behavior (the human-facing table keeps showing stale rows per DATA-01's "flag, don't hide" convention).
- **New `/api/watchlist` route** (`watchlist_json_handler`) mirrors `rankings_json_handler`'s existing shape exactly; touches no catalog/disk I/O (AC #4/NFR3 trivially satisfied — pure in-memory dict read).
- **New `troll/ml_signals/watchlist.py`** (`fetch_watchlist()`) is a separate, dependency-light module (stdlib `urllib`/`json` only) so a backtest script/notebook importing it doesn't pull in `dashboard.py`'s aiohttp/plotly/redis/indicator weight.
- **Task 4's compatibility test does not run a real backtest or touch `backtest_dydx.py`** — confirmed `BacktestDataConfig` construction is I/O-free, so the test builds real `BacktestDataConfig`/`InstrumentId` objects directly from a stubbed `fetch_watchlist()` result and asserts the shape matches, per the story's explicit scope boundary against Epic 2 Story 2.4.
- All 3 lint findings introduced during authoring (2× `D401`, 1× `PYI034`, plus import-sort/format) were fixed except the one documented `D213` case above, which was deliberately left to match existing file convention.

### File List

- `troll/ml_signals/dashboard.py` — added `_WATCHLIST_STALE_NS` constant, `_is_fresh()` helper, `_watchlist_ids()` function, `watchlist_json_handler` route handler, and its registration in `make_app()`
- `troll/ml_signals/watchlist.py` — new file: `fetch_watchlist()`, the lean stdlib-only fetch helper for backtest/notebook consumers
- `troll/ml_signals/tests/test_dashboard_rankings.py` — added 5 new tests: `test_is_fresh_true_within_window`, `test_is_fresh_false_past_stale_window`, `test_is_fresh_false_for_unknown_instrument`, `test_watchlist_ids_excludes_stale_instrument_ac2`, `test_watchlist_ids_sorted_by_descending_volume`
- `troll/ml_signals/tests/test_watchlist.py` — new file: 3 tests (`test_fetch_watchlist_parses_instrument_ids`, `test_fetch_watchlist_empty_list`, `test_watchlist_ids_are_backtest_data_config_compatible`)

## Change Log

- 2026-07-03 — Implemented all 5 tasks: fixed the pre-existing `_LIVE_FAST`-never-evicted gap via a read-time freshness filter (`_is_fresh`, 30s threshold), added `_watchlist_ids()` + `/api/watchlist` route, added the lean `ml_signals/watchlist.fetch_watchlist()` helper, and proved `BacktestDataConfig` compatibility of the returned coin-set. 8 new tests, 186 total passing (1 pre-existing unrelated failure), 1 net-new (deliberately accepted, style-matching) `D213` lint finding. Status → review.
