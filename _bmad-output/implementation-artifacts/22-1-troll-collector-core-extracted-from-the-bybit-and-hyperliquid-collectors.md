...
- [ ] Task 2 — `troll/collector_core/collector.py`: the `Collector` class (AC: #1, #2, #4)
  ...
  - [ ] Candle store, ported from dYdX (`dydx_collector/collector.py`: `_seconds_until_next_flush` wall-clock flush at :02, `_flush_once` -> `_apply_to_candle_store` with exactly the flushed seconds, `_catch_up_candle_store` at startup, `candle_store.prune` in the prune loop): the core owns it; `CANDLES_DB_PATH` is per venue (compose: `candles_dydx.db`, `candles_bybit.db`, `candles_hyperliquid.db`) so three collectors never share one SQLite writer; `data_api` picks the file by `venue_of(instrument_id)`. `seconds_observed` assumes the 1.0 s cadence of AC #3.
