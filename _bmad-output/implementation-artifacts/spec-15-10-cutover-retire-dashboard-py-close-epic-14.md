...
**Block If:**
- **[ALREADY TRUE — this is why this spec is `blocked`]** A dashboard.py page/capability in AC#1's checklist has no verified working React equivalent. Confirmed for **31-day metrics history (Story 15.8)**: `sprint-status.yaml` (commit `0157c2b2e9`, 2026-09-16) deliberately parks 15.8 at `in-progress` — "deferred ... not needed now, per user request ... No dev work has been done on it" — and Story 15.9's own Completion Notes independently confirm `HistoryPage.tsx` "has no metric tiles to apply a box-drawing treatment to — Story 15.8 ... was deferred," i.e. it is still a placeholder, not a working history page. AC#1 requires this row to be confirmed before any deletion proceeds; it cannot be, because closing the gap by shipping 15.8 now would reverse an explicit, recent user decision, and this spec's own **Never** rule below forbids doing that unilaterally.
...
- Do not implement/un-defer Story 15.8 to close this gap unilaterally.
...
**Execution:** blocked before Task 1 completes — no file changes are safe to make until the Block If condition above is resolved by a human.

- [ ] Task 1 (AC#1) — Walk the feature-parity checklist against the running app. **Currently fails on the 31-day metrics history row** (Story 15.8, deliberately deferred, no working equivalent). All other rows (rankings 15.2, chart+candles 15.3, indicator panes 15.4, live edge 15.5, per-coin indicator config 15.6, lines mode 15.7, docs 15.1, terminal identity 15.9) are `done` in `sprint-status.yaml` and plausible from their own Completion Notes, but were not independently re-verified against the running app in this session — that verification is also still owed once the blocker clears.
