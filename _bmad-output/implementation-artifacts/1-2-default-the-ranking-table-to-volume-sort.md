...
- `troll/ml_signals/dashboard.py` — `_VOLUME_24H` cache, volume-24h fetch/poll, `_rankings_json()` sort + raw values, `Vol24h` column, client-side click-to-sort JS; review-fix pass added `network` param to `make_app()`/`app["dydx_network"]` wiring, `DYDX_NETWORK` env var in `__main__`, and NaN/Infinity-safe `raw` cell values in `_rankings_json()`
- `troll/ml_signals/tests/test_dashboard_rankings.py` — new: volume-sort and raw-value tests; review-fix pass added live-resort, non-finite-raw-value, and `make_app` network-wiring tests

### Review Findings

- [x] [Review][Patch] Dashboard always polls dYdX mainnet's volume endpoint, ignoring the collector's configured network — `app["dydx_network"]` is never set anywhere in `make_app()`, so `volume_loop_ctx`'s `app.get("dydx_network", DydxNetwork.MAINNET)` always falls through to the hardcoded default; a testnet deployment silently sorts/labels rows with mainnet volume data [troll/ml_signals/dashboard.py:1042] — **Fixed:** `make_app()` gained a `network: DydxNetwork = DydxNetwork.MAINNET` parameter, sets `app["dydx_network"] = network`; `__main__` reads `DYDX_NETWORK` env var (mirrors the collector's TOML `network` field, defaults to `"mainnet"`) and passes it through. New tests: `test_make_app_wires_configured_network_for_volume_poll`, `test_make_app_defaults_network_to_mainnet`.
...
- [x] [Review][Patch] `"raw": v` passes Python floats straight to `json.dumps` (default `allow_nan=True`) — if any upstream metric is ever NaN/Infinity (e.g. a zero/bad price print reaching `catalog_stats.price_stats`'s pct-change or volatility calc), the response body contains literal `NaN`/`Infinity` tokens that `fetch().json()` cannot parse, breaking the entire rankings table for that poll; a regression from the pre-diff behavior where every value was string-formatted first [troll/ml_signals/dashboard.py:_rankings_json] — **Fixed:** `raw` is now `None` when `v` is a non-finite float (`not math.isfinite(v)`), same treatment as a missing value. New test: `test_rankings_json_cells_null_out_non_finite_raw_value` (asserts no literal `NaN`/`Infinity` substring in the serialized payload and that `json.loads` succeeds).
- [x] [Review][Patch] No test asserts the sort order actually changes between two `_rankings_json()` calls after `_VOLUME_24H` is mutated in between — AC2's "not a one-time computation" isn't directly exercised by any test [troll/ml_signals/tests/test_dashboard_rankings.py] — **Fixed:** no production code change needed (Task 2 already re-sorts on every call) — added `test_rankings_json_sort_updates_live_not_cached`, which calls `_rankings_json()`, mutates `_VOLUME_24H`, calls it again, and asserts the order actually flips.
...
- 2026-07-03 — Review-fix pass: addressed all 3 open `[Review][Patch]` findings. `make_app()`/`__main__` now wire a configurable `DydxNetwork` (env var `DYDX_NETWORK`) into the volume-24h poll instead of always defaulting to mainnet; `_rankings_json()` nulls out non-finite (`NaN`/`Infinity`) `raw` cell values before `json.dumps` to prevent malformed-JSON responses; added a regression test proving the volume sort re-evaluates on every call rather than caching a one-time order. 4 new tests (`test_rankings_json_sort_updates_live_not_cached`, `test_rankings_json_cells_null_out_non_finite_raw_value`, `test_make_app_wires_configured_network_for_volume_poll`, `test_make_app_defaults_network_to_mainnet`); full suite 178 passed (up from 174), same 1 pre-existing unrelated `test_ofi_strategy.py` failure; `ruff check`/`ruff format --check` on both touched files reconfirmed at the same pre-existing finding count as before this pass (0 new), via `git stash` diff. Status → done.
