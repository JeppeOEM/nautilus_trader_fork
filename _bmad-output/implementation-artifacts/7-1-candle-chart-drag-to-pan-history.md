...
- [ ] [Review][Decision] AC #1 pan/scroll-zoom not wired for Lines mode — Lines is `_coinMode`'s default on `showCoin()`; its `Plotly.react` call (dashboard.py ~657-663) has no `dragmode:'pan'`/`scrollZoom` and never calls `_wireChartRelayout()`. AC #1 is worded chart-generically ("the coin chart... a left-click-drag moves the visible time window"), but Task 2's own subtask scoped the work to `_renderCandleChart`/`_renderTickChart` only. Confirm this narrowing is intentional or extend AC #1 to Lines too.
- [ ] [Review][Decision] AC #2's "Plotly.react preserves zoom on redraw" is unverified in a real browser — self-flagged in Completion Notes; the mock-Plotly Node harness used for JS verification cannot observe this behavior. Confirm acceptance criteria (browser spot-check) or accept as a tracked risk.
- [ ] [Review][Decision] Live (mid-price) and historical (trade-price) candles are concatenated with no seam handling — panning left on a live candle session (never clicked "Load") silently switches OHLC price basis mid-series at the prepend boundary, a visible discontinuity. Options: (a) on the first pan-triggered backward fetch of a live session, also refetch the currently-displayed window via `_historical_candles_json` so the whole series is trade-price-based going forward, or (b) accept the seam as a known limitation.
- [ ] [Review][Decision] Zero test coverage for the new client-side pan/pagination state machine (`_chartState`, `_loadOlderChunk`, `_onChartRelayout`, `_maybeLoadOlder`) — real branching/race logic per TEST-01's spirit, but no JS test framework precedent exists in this repo (Python is the only tested layer per project convention, confirmed in this story's own Completion Notes). Introduce a minimal JS test harness now, or accept as a known gap consistent with existing convention.
- [ ] [Review][Patch] `_coinHistStart='panning'` sentinel breaks bar/mode change while frozen from a drag [dashboard.py:421] — `onBarChange()`/`setCoinMode()` (dashboard.py:331,342) feed the literal string `'panning'` into `_fetchHistCoin` as if it were a real date, producing `NaN`/epoch timestamps; violates AC #5 ("stale candles from a different bar size must never linger").
- [ ] [Review][Patch] `_loadOlderChunk` has no request-generation guard [dashboard.py:394-408] — reads/mutates the global `_chartState` at promise-resolution time rather than a value captured when the fetch started; switching coin/mode/bar while a backward fetch is in flight can splice stale rows into a different chart.
- [ ] [Review][Patch] `_historical_ticks_json` materializes the full unbounded catalog result before applying `max_rows` [dashboard.py:883-904] — `catalog.trade_ticks(...)` reads the entire `[start_ms,end_ms]` window before `[:max_rows]` truncates it; a wide date range via the existing Load button while in Ticks mode drives an unbounded in-memory read (MEM-01).
- [ ] [Review][Patch] `_chunkSpanMs()`'s candle formula blows up for the newly-added 1d/1w bar options [dashboard.py:391-393] — `barSeconds*1000*200` with `barSeconds=604800` (1w) requests a ~10-year-wide chunk per pan step.
- [ ] [Review][Patch] Tick row-cap truncation silently drops trades and the cursor still advances past the clamped edge [dashboard.py:883-904, 394-408] — once truncated, subsequently-dropped trades become permanently unreachable with no signal to the user.
- [ ] [Review][Patch] `plotly_relayout` listener never detached when switching back to Lines mode [dashboard.py:657-663] — a drag/box-select on the Lines chart can still fire `_onChartRelayout` and freeze live polling with no visible cause.
- [ ] [Review][Patch] `_parse_ms` duplicated verbatim across `coin_candles_handler`/`coin_ticks_handler` [dashboard.py:1146-1153, 1169-1176].
- [ ] [Review][Patch] Tick coloring defaults anything other than `'BUYER'` (including `NO_AGGRESSOR`) to sell-red [dashboard.py:463].
- [ ] [Review][Patch] Live-candles fetch chain in `renderCoin` has no `.catch`, unlike the sibling ticks fetch added right next to it [dashboard.py:632-634].
