...
**Multi-Venue Market Data Collector** (`troll/collector_core/` + `troll/dydx_collector/`, `troll/bybit_collector/`, `troll/hyperliquid_collector/`)

Standalone Python asyncio services — one per venue, all three subclassing the one shared `collector_core.collector.Collector` (Epic 22) — that connect directly to each venue's Rust/PyO3-backed HTTP and WebSocket clients (dYdX's `nautilus_pyo3.DydxHttpClient`/`DydxWebSocketClient`, and the Bybit and Hyperliquid equivalents, each wrapped by that venue package's own duck-typed `client.py`) — bypassing `TradingNode`/`Strategy`/`DataEngine` entirely — and continuously archive the available market data (1-second book/trade snapshots, mark/index price, funding rate, open interest, instrument definitions — plus raw order book deltas for instruments opted in via `store_order_book_deltas`, which is a **dYdX-only** option — only `DydxConfig`'s per-instrument entry has it; Bybit's and Hyperliquid's `instruments` are a flat `tuple[str, ...]`. Individual trades are folded into the second's snapshot rather than stored raw, audit D-45. What each venue actually yields differs: Bybit **spot** gets trades and book only — `bybit_collector/client.py:124-125` subscribes the ticker for `LINEAR` alone, so a spot id produces no mark/index price and no funding rate, and `bybit_collector/open_interest.py` builds `-LINEAR.BYBIT` ids only) to one shared `ParquetDataCatalog` in the exact format Nautilus expects, with zero conversion step. Runs in Docker alongside Dozzle for live log visibility.

**Core Value:** Reliable, continuous capture of dYdX, Bybit and Hyperliquid market data into one Nautilus-catalog-compatible Parquet archive, decoupled from Nautilus's live-trading runtime — which has a documented unbounded-queue-growth + shutdown-wedge bug under sustained high-message-load (see `nautilus_trader/live/data_engine.py` + `live/enqueue.py`'s `ThrottledEnqueuer`) that previously OOM-crashed an earlier `Strategy`/`TradingNode`-based recorder on the `gg` branch. Each collector reuses its venue adapter's Rust connection/reconnect/throttle/decode logic directly (own asyncio loop, own callback) so it gets that battle-tested networking for free without inheriting the buggy `DataEngine` layer.
...
- **Architecture**: No `TradingNode`/`Strategy`/`DataEngine` — a plain asyncio class (`troll/collector_core/collector.py`'s `Collector`, subclassed by `DydxCollector`/`BybitCollector`/`HyperliquidCollector`) owning its own loop, buffer, and flush timer. A venue package supplies a duck-typed client plus at most a few hook overrides; the write gate itself is never overridden. `nautilus_trader` is used purely as a library (domain types + `ParquetDataCatalog.write_data()`), never as a live runtime.
- **Repo location**: Lives inside `nautilus_trader_fork`, under `troll/`, on the `pony` branch — co-located with this repo's git history rather than a separate repo
...
- **Deployment**: Docker / Docker Compose. One durable base image, `nautilus-trader-base`, built from `.docker/nautilus_trader.dockerfile`'s `application` target (rebuilt rarely — only when `nautilus_trader` core/deps change), carries three thin layers that rebuild in seconds: `troll/collector.dockerfile` (bakes `collector_core`, the three venue collectors, `common`, `ml_signals`, `ranking_engine`, `bot_tui`, `data_api` — one image, one service per collector, different `command:`, all three writing the same catalog root), `troll/data_api.dockerfile` (separate because it also runs a Node frontend-build stage and bakes its own package set) and `troll/live_paper.dockerfile`. Plus a Dozzle container for log viewing (`troll/docker-compose.yml`).
- **Fork safety**: Never modify `nautilus_trader/` or `crates/` — the collector is new, additive code only
- **Open interest**: how it arrives differs per venue and is a per-venue investigation, never an assumption. dYdX: dropped by the Rust/PyO3 bindings on both the REST and WS markets-channel paths, so it is fetched separately via a plain stdlib `urllib` poll against dYdX's public indexer REST endpoint (`troll/dydx_collector/open_interest.py`). Bybit: dropped on the linear-ticker WS path, so likewise a REST poll (`troll/bybit_collector/open_interest.py`); Bybit **spot** has no open interest at all, by construction. Hyperliquid: forwarded over the WebSocket (`subscribe_open_interest`), so no poll. All of them land in one custom `Data` type registered for Arrow/Parquet serialization — since story 22.3 the one shared `collector_core.open_interest.OpenInterest` for every venue, replacing the per-venue `DydxOpenInterest`/`BybitOpenInterest`/`HyperliquidOpenInterest` classes
- **Rate limits**: dYdX relies on the Rust WebSocket client's existing built-in subscribe-throttle (dYdX's 2/sec limit) and reconnect handling — no custom throttling code needed. Bybit and Hyperliquid needed no cap for the instrument counts collected (stories 19.3/19.4); a new venue's limits are part of the wire-behaviour investigation `troll/CLAUDE.md`'s "Adding a venue" step 1 requires, not something inherited from dYdX
- **Price/quantity integrity**: Never derive a value's stored precision from its own digit count (e.g. via `Decimal.normalize()`), and never round-trip a market data value through `float` before it's safely inside a `Price`/`Quantity`. Real incident: dYdX's mark/index price feed derives each tick's `Price.precision` from however many decimals that specific value has after stripping trailing zeros (`crates/adapters/dydx/src/common/parse.rs`'s `parse_price`), so consecutive ticks for one instrument can carry different precision labels — `ParquetDataCatalog` correctly refuses to read/merge files whose labels disagree. Fixed in `troll/dydx_collector/client.py`'s `_at_fixed_precision()` (dYdX-only: Bybit and Hyperliquid parse mark/index at the instrument's own constant precision, so neither needs re-stamping), which re-stamps every mark/index price at `nautilus_pyo3.FIXED_PRECISION` via `Decimal.scaleb()` + `Price.from_raw()` (exact integer arithmetic, no float). Also watch for: `Price(decimal, precision)` has a real bug in this nautilus_trader version for some decimal/precision combinations — `Price(Decimal("61090.59855"), 16)` silently returns `61090.5985500000026624`. Use `Decimal.scaleb()` + `Price.from_raw()`/`Quantity.from_raw()` instead whenever re-stamping a value at a different precision.
...
- **Use Nautilus built-ins first.** Before writing custom data loading, scheduling, reporting, aggregation, or indicator code, check whether `BacktestNode`, `BacktestDataConfig`, `TradingNode`, `DataClient`, or another Nautilus primitive already covers it. Wrapping Nautilus is correct; duplicating it is not.
  - **Indicators specifically:** `nautilus_trader.indicators` ships a full streaming (Cython, `O(1)` per-event `update_raw()`, no history rescan) TA library — SMA/EMA/WMA/Hull/Adaptive MA, RSI, MACD, Stochastics, CCI, ATR, Bollinger/Donchian/Keltner, OBV, VWAP, Ichimoku, and more. Reach for these before `pandas-ta` or a hand-rolled version of a classic indicator — `pandas-ta` is batch/DataFrame-oriented and doesn't fit Nautilus's per-bar streaming model anyway. Only write a custom `Indicator` subclass (as `troll/ml_signals/indicators.py` already does for OFI/OBI/microprice) when no built-in covers the signal.
