...
- **Deployment:** Docker Compose (`platform/docker-compose.yml`). One durable base image,
...
- **Rate limits:** dYdX relies on the Rust WebSocket client's built-in subscribe throttle
  ...
  the wire-behaviour investigation in `platform/CLAUDE.md`'s "Adding a venue", not inherited.
...
`platform/CLAUDE.md` holds the binding rules for all code under `platform/`: fork safety, data
...
`nautilus_trader`/`crates/` codebase are in `platform/docs/NAUTILUS_UPSTREAM_REFERENCE.md`. It is
